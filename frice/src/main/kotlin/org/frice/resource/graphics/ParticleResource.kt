...
constructor(
	 ...
	 */
	private val image = DroidImage(width, height)
	...
	private fun drawBackground() {
		...
		when (back) {
			is ColorResource -> {
				g.color = back.`get-color`()
				g.fillRect(0, 0, width, height)
			}
			is ImageResource -> back.image.let { g.drawImage(cast<DroidImage>(it).image, 0, 0, width, height, game) }
		}
	}
